...
x = tf.placeholder('float'[None, 784])#heightx width it shows error when it goes over this limit
y = tf.placeholder('float')

def = neutak_network_model(data):
...
def train_neural_network(x):
	prediction = neutak_network_model(x)
...
'''
	...
	with tf.Session() as sess:
		...
		print('Accuracy:', accuracy.eval({x:mist.test.images, y:mnist.test.labels}))
